...
describe("live VPN session grouping", () => {
  it("combines parallel Cloudflare tunnels with the same client, source network, and protocol", () => {
    ...
    expect(groups.find(group => group.protocol === "vless")).toMatchObject({ tunnelCount: 2, uplinkBytes: 400, downlinkBytes: 600, startedAt: new Date("2026-08-20T07:59:00.000Z"), lastSeenAt: new Date("2026-08-20T08:02:00.000Z") });
    expect(groups.find(group => group.protocol === "xhttp")).toMatchObject({ tunnelCount: 1, uplinkBytes: 5, downlinkBytes: 6 });
  });
});
