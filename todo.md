...
- [x] Add Shadowsocks 2022 over WebSocket globally and per named client, including credentials, opaque routes, Xray runtime, subscription/QR output, and existing quota, speed, connection, activation, storage, and day-limit enforcement.
...
- [x] Deferred at owner request: diagnose or change the reported unlimited-client throughput ceiling.
- [x] Deferred at owner request: benchmark the Azure VPN path for a gateway optimization.
- [x] Deferred at owner request: pursue an external-to-gateway throughput isolation or cap removal.
