...
describe("admin sidebar navigation", () => {
  it("maps all owner workspaces to distinct client-side routes", () => {
    expect(adminRoutes).toEqual({ overview: "/admin", clients: "/admin/clients", liveControl: "/admin/live", devices: "/admin/devices", templates: "/admin/templates", recovery: "/admin/recovery", terminal: "/admin/terminal" });
    expect(adminRouteFor("overview")).toBe("/admin");
    expect(adminRouteFor("clients")).toBe("/admin/clients");
    expect(adminRouteFor("liveControl")).toBe("/admin/live");
