...
export function ClientRegistry({ clients, pending, onToggle, onActivate, onRotate, onResetUsage, onDelete, onCopy, onSavePolicy }: ClientRegistryProps) {
  ...
  if (!clients.length) {
    return <div className="rounded-xl border border-dashed border-border bg-card/30 px-5 py-14 text-center"><HardDrive className="mx-auto h-7 w-7 text-muted-foreground" /><h3 className="mt-3 font-medium">No named clients yet</h3><p className="mt-1 text-sm text-muted-foreground">Create an identity above to issue independent credentials and a subscription feed.</p></div>;
  }
