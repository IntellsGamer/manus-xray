...
import { CalendarClock, Copy, Gauge, HardDrive, KeyRound, MoreHorizontal, Power, RotateCcw, Save, Trash2 } from "lucide-react";
...
type ClientRegistryProps = {
  ...
  onSavePolicy: (id: number, trafficLimitBytes: number, dayLimit: number, speedLimitMbps: number) => void;
};
...
function clientStatus(client: ClientRecord) {
  if (client.activationPending) return { label: "Activating shortly", detail: "Credentials saved; Xray refresh is deferred", className: "border-sky-500/35 bg-sky-500/10 text-sky-700 dark:text-sky-300" };
  if (client.enabled) return { label: "Enabled", detail: "Ready for connections", className: "border-emerald-500/30 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300" };
  if (client.quotaExhaustedAt) return { label: "Quota reached", detail: "Disabled by quota policy", className: "border-amber-500/35 bg-amber-500/10 text-amber-700 dark:text-amber-300" };
  return { label: "Disabled", detail: "Connections are blocked", className: "border-muted-foreground/30 text-muted-foreground" };
}
