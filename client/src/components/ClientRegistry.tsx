...
export function compactUnlimitedExpiryDetail(dayLimit: number) {
  return dayLimit < 0 ? "Expires never" : undefined;
}
...
export function compactSpeedPolicy(speedLimitMbps: number) {
  return speedLimitMbps < 0 ? "Unlimited" : `${speedLimitMbps} Mbps`;
}

export function compactSpeedToneClass(speedLimitMbps: number) {
  return speedLimitMbps < 0 ? "text-muted-foreground" : "text-foreground";
}

export function compactQuotaUsageText(trafficUsedBytes: number, trafficLimitBytes: number) {
  return `${formatBytes(Math.max(0, trafficUsedBytes))} / ${formatBytes(trafficLimitBytes)}`;
}
...
export const compactClientCardGridClass = "grid gap-3 px-4 py-3 lg:grid-cols-[minmax(10rem,auto)_minmax(14rem,1fr)_auto] lg:items-center lg:gap-5";
export const compactClientActionContainerClass = "w-full lg:w-auto lg:justify-self-end";
export const expandableClientActionsClass = "flex w-full items-center gap-2 lg:w-auto";
export const compactExpansionButtonClass = "ml-auto h-9 w-9 lg:ml-0";
export const detailedClientCardGridClass = "grid gap-3 p-5 sm:p-6 lg:grid-cols-[minmax(10rem,auto)_minmax(13rem,1fr)_auto] lg:items-center lg:gap-5";
export const detailedClientActionContainerClass = "w-full lg:col-start-3 lg:w-auto lg:justify-self-end";
...
function CompactSpeedSummary({ speedLimitMbps, className = "mt-1.5" }: { speedLimitMbps: number; className?: string }) {
  const toneClass = compactSpeedToneClass(speedLimitMbps);
  return <div className={`${className} flex items-center justify-end gap-1.5 text-[11px] font-medium tabular-nums ${toneClass}`}><Gauge className="h-3.5 w-3.5" aria-hidden="true" /><span>{compactSpeedPolicy(speedLimitMbps)}</span></div>;
}

function QuotaProgressBar({ quotaProgress, compact = false, speedLimitMbps, trafficUsedBytes, trafficLimitBytes }: { quotaProgress: QuotaProgress; compact?: boolean; speedLimitMbps?: number; trafficUsedBytes?: number; trafficLimitBytes?: number }) {
...
function ClientIdentity({ client, status, compact, validity }: { client: ClientRecord; status: ReturnType<typeof clientActivationState>; compact: boolean; validity: ReturnType<typeof dayStatus> }) {
  const detail = client.activationPending || client.activationFailed ? status.detail : compact ? compactUnlimitedExpiryDetail(client.dayLimit) ?? validity.detail : `Created ${new Date(client.createdAt).toLocaleDateString()}`;
