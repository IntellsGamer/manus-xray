...
function QuotaProgressBar({ quotaProgress, compact = false, summary }: { quotaProgress: QuotaProgress; compact?: boolean; summary?: string }) {
...
export function ClientRegistry({ clients, viewMode, pending, onToggle, onActivate, onRotate, onResetUsage, onDelete, onCopy, onSavePolicy }: ClientRegistryProps) {
  ...
  return <div className={viewMode === "compact" ? "space-y-2" : "space-y-3"}>
    {clients.map(client => {
      ...
      if (viewMode === "compact" && !expanded) return <article key={client.id} className="overflow-hidden rounded-xl border border-border bg-card/45 shadow-sm transition-[border-color,box-shadow] duration-200"><div className="grid gap-3 px-4 py-3 sm:grid-cols-[minmax(10rem,auto)_minmax(14rem,1fr)_auto] sm:items-center sm:gap-5"><ClientIdentity client={client} status={status} compact validity={validity} />{quotaProgress ? <QuotaProgressBar quotaProgress={quotaProgress} compact summary={compactSpeedPolicy(client.speedLimitMbps)} /> : <div className="min-w-0"><div className="flex items-center justify-between gap-3 text-[11px] font-medium"><span className="text-muted-foreground">Data usage</span><span className="tabular-nums text-foreground">{compactSpeedPolicy(client.speedLimitMbps)}</span></div><p className="mt-1 text-xs text-muted-foreground">{formatBytes(client.trafficUsedBytes)} used</p></div>}<div className="sm:justify-self-end">{actions}</div></div></article>;
