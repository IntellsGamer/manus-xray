...
function CompactSpeedSummary({ speedLimitMbps }: { speedLimitMbps: number }) {
  const toneClass = compactSpeedToneClass(speedLimitMbps);
  return <div className={`mt-1.5 flex items-center justify-end gap-1.5 text-[11px] font-medium tabular-nums ${toneClass}`}><Gauge className="h-3.5 w-3.5" aria-hidden="true" /><span>{compactSpeedPolicy(speedLimitMbps)}</span></div>;
}

function QuotaProgressBar({ quotaProgress, compact = false, speedLimitMbps }: { quotaProgress: QuotaProgress; compact?: boolean; speedLimitMbps?: number }) {
