...
export function ClientRegistry({ clients, pending, onToggle, onActivate, onRotate, onResetUsage, onDelete, onCopy, onSavePolicy }: ClientRegistryProps) {
  ...
  return <div className="space-y-3">{clients.map(client => {
    ...
    const connectionDetail = client.connectionLimit < 0 ? { title: "Unlimited", detail: "No concurrent tunnel cap" } : { title: `${client.connectionLimit} concurrent`, detail: "Shared across all protocols" };
