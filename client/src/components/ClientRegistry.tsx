...
function QuotaProgressBar({ quotaProgress, compact = false, speedPolicy }: { quotaProgress: QuotaProgress; compact?: boolean; speedPolicy?: string }) {
...
export function ClientRegistry({ clients, viewMode, pending, onToggle, onActivate, onRotate, onResetUsage, onDelete, onCopy, onSavePolicy }: ClientRegistryProps) {
  ...
  return <div className={viewMode === "compact" ? "space-y-2" : "space-y-3"}>
    {clients.map(client => {
      ...
      if (viewMode === "compact" && !expanded) return <article key={client.id} className="overflow-hidden rounded-xl border border-border bg-card/45 shadow-sm transition-[border-color,box-shadow] duration-200"><div className="grid gap-3 px-4 py-3 sm:grid-cols-[minmax(10rem,auto)_minmax(14rem,1fr)_auto] sm:items-center sm:gap-5"><ClientIdentity client={client} status={status} compact validity={validity} />{quotaProgress ? <QuotaProgressBar quotaProgress={quotaProgress} compact speedPolicy={compactSpeedPolicy(client.speedLimitMbps)} /> : <div className="min-w-0"><div className="text-[11px] font-medium text-muted-foreground">Data usage</div><p className="mt-1 text-xs text-muted-foreground">{formatBytes(client.trafficUsedBytes)} used</p><div className="mt-1.5 flex items-center justify-between gap-3 text-[11px]"><span className="text-muted-foreground">Speed</span><span className="shrink-0 font-medium tabular-nums text-foreground">{compactSpeedPolicy(client.speedLimitMbps)}</span></div></div>}<div className="sm:justify-self-end">{actions}</div></div></article>;
