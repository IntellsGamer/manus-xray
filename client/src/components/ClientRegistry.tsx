...
function ClientActions({ client, pending, status, subscriptionUrl, onToggle, onActivate, onRotate, onReset, onDelete, onCopy }: { client: ClientRecord; pending: boolean; status: ReturnType<typeof clientActivationState>; subscriptionUrl: string; onToggle: (id: number, enabled: boolean) => void; onActivate: (id: number) => void; onRotate: (id: number) => void; onReset: (candidate: ClientActionCandidate) => void; onDelete: (candidate: ClientActionCandidate) => void; onCopy: (value: string) => void }) {
...
export function ClientRegistry({ clients, viewMode, pending, onToggle, onActivate, onRotate, onResetUsage, onDelete, onCopy, onSavePolicy }: ClientRegistryProps) {
  ...
  return <div className={viewMode === "compact" ? "space-y-2" : "space-y-3"}>
    {clients.map(client => {
      ...
      const actions = <ClientActions client={client} pending={pending} status={status} subscriptionUrl={subscriptionUrl} onToggle={onToggle} onActivate={onActivate} onRotate={onRotate} onReset={setResetCandidate} onDelete={setDeleteCandidate} onCopy={onCopy} />;

      if (viewMode === "compact") return <article key={client.id} className="overflow-hidden rounded-xl border border-border bg-card/45 shadow-sm"><div className="grid gap-3 px-4 py-3 sm:grid-cols-[minmax(10rem,auto)_minmax(14rem,1fr)_auto] sm:items-center sm:gap-5"><ClientIdentity client={client} status={status} compact validity={validity} />{quotaProgress ? <QuotaProgressBar quotaProgress={quotaProgress} compact /> : <div className="min-w-0"><div className="flex items-center justify-between gap-3 text-[11px] font-medium"><span className="text-muted-foreground">Data usage</span><span className="tabular-nums text-foreground">Unlimited</span></div><p className="mt-1 text-xs text-muted-foreground">{formatBytes(client.trafficUsedBytes)} used</p></div>}<div className="sm:justify-self-end">{actions}</div></div></article>;
