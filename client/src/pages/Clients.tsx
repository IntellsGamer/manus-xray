...
export function ClientManagerContent() {
  ...
  return <div className="protocol-shell min-h-full"><div className="mx-auto max-w-6xl space-y-5 py-2 sm:py-5">
    ...
    <section className="grid gap-5 xl:grid-cols-[1.1fr_.9fr]"><div className="protocol-panel p-5 sm:p-6"><div className="flex items-start justify-between"><div><p className="protocol-overline">Create identity</p><h2 className="mt-2 text-lg font-semibold">New client profile</h2><p className="mt-1 text-sm text-muted-foreground">Creates separate VLESS, VMess, Trojan, SOCKS5, and subscription credentials.</p></div><Plus className="h-5 w-5 text-primary" /></div><form className="mt-5 grid gap-3 sm:grid-cols-2" onSubmit={(event: FormEvent) => { event.preventDefault(); if (!name.trim()) return; const parsedDays = Number(dayLimit); create.mutate({ name, trafficLimitBytes: limitToBytes(trafficLimit, trafficUnit), dayLimit: !Number.isFinite(parsedDays) || parsedDays === -1 ? -1 : Math.max(0, Math.min(3650, Math.floor(parsedDays))) }); }}><Input value={name} onChange={event => setName(event.target.value)} placeholder="Client or device label" className="protocol-input" /><Input value={trafficLimit} onChange={event => setTrafficLimit(event.target.value)} type="number" min="-1" step="0.01" className="protocol-input" aria-label="Storage quota" /><select value={trafficUnit} onChange={event => setTrafficUnit(event.target.value as QuotaUnit)} className="protocol-input h-10 rounded-md border border-input bg-background px-2 text-sm" aria-label="Storage quota unit"><option value="GB">GB</option><option value="MB">MB</option></select><Input value={dayLimit} onChange={event => setDayLimit(event.target.value)} type="number" min="-1" max="3650" className="protocol-input" aria-label="Day limit" /><Button className="sm:w-fit" disabled={create.isPending}><Plus className="mr-2 h-4 w-4" />Create</Button></form><p className="mt-3 text-xs text-muted-foreground"><strong>-1</strong> means unlimited for storage and days.</p></div>
    ...
    <section className="space-y-3"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="protocol-overline">Named identities</p><h2 className="mt-1 text-lg font-semibold">Client lifecycle & policy registry</h2><p className="mt-1 text-sm text-muted-foreground">Quota values use Xray’s local per-user counters when the runtime sampler is available.</p></div><div className="rounded-md bg-muted px-3 py-1.5 text-xs text-muted-foreground">{clients?.filter(client => client.enabled).length || 0} enabled · {clients?.length || 0} total</div></div><ClientRegistry clients={clients || []} pending={toggle.isPending || rotate.isPending || deleteClient.isPending || updatePolicy.isPending} onToggle={(id, enabled) => toggle.mutate({ id, enabled })} onRotate={id => rotate.mutate({ id })} onDelete={id => deleteClient.mutate({ id })} onCopy={value => copy(value, "Subscription URL")} onSavePolicy={(id, trafficLimitBytes, policyDayLimit) => updatePolicy.mutate({ id, trafficLimitBytes, dayLimit: policyDayLimit })} /></section>
  </div></div>;
}
