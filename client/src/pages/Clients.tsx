...
import { LiveSessionsPanel, type ClientProtocol } from "@/components/LiveSessionsPanel";
...
export function ClientManagerContent() {
  ...
  const { data: liveSessions, isFetching: fetchingLiveSessions } = trpc.vless.liveSessions.useQuery(undefined, { retry: false, refetchInterval: 3_000 });
  ...
  const [paths, setPaths] = useState({ wsPath: "/vless", vmessWsPath: "/vmess", trojanWsPath: "/trojan", socksWsPath: "/socks", shadowsocksWsPath: "/shadowsocks", globalProfileEnabled: true });
  ...
  useEffect(() => {
    if (!profile) return;
    setPaths({ wsPath: profile.wsPath, vmessWsPath: profile.vmessWsPath, trojanWsPath: profile.trojanWsPath, socksWsPath: profile.socksWsPath, shadowsocksWsPath: profile.shadowsocksWsPath, globalProfileEnabled: profile.globalProfileEnabled });
  ...
  const refresh = () => { utils.vless.get.invalidate(); utils.vless.clients.invalidate(); utils.vless.liveSessions.invalidate(); };
  ...
  const updatePaths = trpc.vless.updatePaths.useMutation({
    onSuccess: saved => { setPaths({ wsPath: saved.wsPath, vmessWsPath: saved.vmessWsPath, trojanWsPath: saved.trojanWsPath, socksWsPath: saved.socksWsPath, shadowsocksWsPath: saved.shadowsocksWsPath, globalProfileEnabled: saved.globalProfileEnabled }); setSavedGlobalProfileEnabled(saved.globalProfileEnabled); toast.success("Gateway paths saved"); refresh(); },
    onError: error => toast.error(error.message),
  });
  const persistGlobalProfile = trpc.vless.updatePaths.useMutation({
    onSuccess: saved => { setPaths({ wsPath: saved.wsPath, vmessWsPath: saved.vmessWsPath, trojanWsPath: saved.trojanWsPath, socksWsPath: saved.socksWsPath, shadowsocksWsPath: saved.shadowsocksWsPath, globalProfileEnabled: saved.globalProfileEnabled }); setSavedGlobalProfileEnabled(saved.globalProfileEnabled); toast.success(saved.globalProfileEnabled ? "Legacy credentials enabled" : "Legacy credentials disabled"); refresh(); },
  ...
  const disconnectLiveSession = trpc.vless.disconnectLiveSession.useMutation({ onSuccess: () => { toast.success("Disconnect requested for the live tunnel"); utils.vless.liveSessions.invalidate(); }, onError: error => toast.error(error.message) });
  ...
  return <div className="protocol-shell min-h-full"><div className="mx-auto max-w-6xl space-y-5 py-2 sm:py-5">
    ...
    <section className="protocol-panel p-5 sm:p-6"><div className="flex items-center gap-3"><Route className="h-5 w-5 text-primary" /><div><p className="protocol-overline">Transport paths</p><h2 className="mt-1 text-lg font-semibold">Independent protocol routes</h2></div></div><div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">{([['VLESS', 'wsPath'], ['VMess', 'vmessWsPath'], ['Trojan', 'trojanWsPath'], ['SOCKS5', 'socksWsPath'], ['Shadowsocks 2022', 'shadowsocksWsPath']] as const).map(([label, key]) => <div key={key} className="space-y-2"><Label>{label} path</Label><Input value={paths[key]} onChange={event => setPaths(current => ({ ...current, [key]: event.target.value }))} className="protocol-input font-mono" /></div>)}</div><div className="mt-5 flex justify-end"><Button onClick={() => updatePaths.mutate(paths)} disabled={updatePaths.isPending}>Save routes</Button></div></section>

    <LiveSessionsPanel clients={clients || []} sessions={liveSessions || []} pending={disconnectLiveSession.isPending || updatePolicy.isPending} onDisconnect={id => disconnectLiveSession.mutate({ id })} onSaveAllowedProtocols={(id, allowedProtocols) => { const client = clients?.find(item => item.id === id); if (!client) return; updatePolicy.mutate({ id, trafficLimitBytes: client.trafficLimitBytes, dayLimit: client.dayLimit, speedLimitMbps: client.speedLimitMbps, connectionLimit: client.connectionLimit, allowedProtocols }); }} />

    <section className="space-y-3"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="protocol-overline">Named identities</p><h2 className="mt-1 text-lg font-semibold">Client lifecycle & policy registry</h2><p className="mt-1 text-sm text-muted-foreground">Quota values update in real time from payload bytes crossing each route-identified gateway tunnel.</p></div><div className="flex items-center gap-2"><div className="rounded-md bg-muted px-3 py-1.5 text-xs text-muted-foreground">{clients?.filter(client => client.enabled).length || 0} enabled · {clients?.length || 0} total</div><div className="flex rounded-md border border-border bg-card p-0.5" aria-label="Client display mode"><Button type="button" size="icon" variant={clientRegistryView === "detailed" ? "secondary" : "ghost"} className="h-7 w-7" aria-label="Detailed client cards" title="Detailed cards" onClick={() => setRegistryView("detailed")}><Rows3 className="h-3.5 w-3.5" /></Button><Button type="button" size="icon" variant={clientRegistryView === "compact" ? "secondary" : "ghost"} className="h-7 w-7" aria-label="Compact client cards" title="Compact cards" onClick={() => setRegistryView("compact")}><List className="h-3.5 w-3.5" /></Button></div><Button type="button" size="icon" variant="outline" className="h-8 w-8" aria-label="Refresh client registry" title="Refresh client registry" disabled={fetchingProfile || fetchingClients || fetchingLiveSessions} onClick={refresh}><RefreshCw className={`h-3.5 w-3.5 ${(fetchingProfile || fetchingClients || fetchingLiveSessions) ? "animate-spin" : ""}`} /></Button></div></div><ClientRegistry clients={clients || []} viewMode={clientRegistryView} pending={activate.isPending || toggle.isPending || rotate.isPending || resetUsage.isPending || deleteClient.isPending || updatePolicy.isPending || disconnectLiveSession.isPending} onToggle={(id, enabled) => toggle.mutate({ id, enabled })} onActivate={id => activate.mutate({ id, force: true })} onRotate={id => rotate.mutate({ id })} onResetUsage={id => resetUsage.mutate({ id })} onDelete={id => deleteClient.mutate({ id })} onCopy={value => copy(value, "Subscription URL")} onSavePolicy={(id, trafficLimitBytes, policyDayLimit, policySpeedLimitMbps, policyConnectionLimit) => updatePolicy.mutate({ id, trafficLimitBytes, dayLimit: policyDayLimit, speedLimitMbps: policySpeedLimitMbps, connectionLimit: policyConnectionLimit })} /></section>
  </div></div>;
}
