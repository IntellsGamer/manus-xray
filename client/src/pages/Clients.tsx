...
export function ClientManagerContent() {
  ...
  return <div className="protocol-shell min-h-full"><div className="mx-auto max-w-6xl space-y-5 py-2 sm:py-5">
    ...
    <section className="grid gap-5 xl:grid-cols-[1.1fr_.9fr]"><div className="protocol-panel p-5 sm:p-6"><div className="flex items-start justify-between"><div><p className="protocol-overline">Create identity</p><h2 className="mt-2 text-lg font-semibold">New client profile</h2><p className="mt-1 text-sm text-muted-foreground">Creates separate VLESS, VMess, Trojan, SOCKS5, and subscription credentials.</p></div><Plus className="h-5 w-5 text-primary" /></div><form className="mt-5 flex gap-3" onSubmit={(event: FormEvent) => { event.preventDefault(); if (name.trim()) create.mutate({ name }); }}><Input value={name} onChange={event => setName(event.target.value)} placeholder="e.g. Device A" className="protocol-input" /><Button disabled={create.isPending}><Plus className="mr-2 h-4 w-4" />Create</Button></form></div>
