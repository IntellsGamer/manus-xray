...
import { ClientManagerContent } from "@/pages/Clients";
...
type ProtocolKey = "vless" | "vmess" | "trojan" | "socks";
...
function AdminContent() {
  ...
  useEffect(() => {
    ...
    if (sectionId === "gateway-overview" || sectionId === "client-management") {
      window.setTimeout(() => document.getElementById(sectionId)?.scrollIntoView({ behavior: "auto", block: "start" }), 0);
    }
  }, []);
  ...
  const activeUri = activeProtocol === "vless" ? profile.vlessUri : activeProtocol === "vmess" ? profile.vmess.uri : activeProtocol === "trojan" ? profile.trojan.uri : profile.socks5.clientConfig;
  const activeMeta = protocolMeta[activeProtocol];
  const activePath = activeProtocol === "vless" ? profile.wsPath : activeProtocol === "vmess" ? profile.vmess.wsPath : activeProtocol === "trojan" ? profile.trojan.wsPath : profile.socks5.wsPath;
  ...
  return <div id="gateway-overview" className="protocol-shell min-h-full"><QrImportDialog payload={qrPayload} onOpenChange={open => !open && setQrPayload(null)} /><div className="mx-auto max-w-7xl px-1 py-2 sm:px-4 sm:py-5">
    <header className="protocol-header"><div><div className="mb-3 flex items-center gap-2 text-primary"><ServerCog className="h-4 w-4" /><span className="protocol-overline">Nginx gateway</span></div><h1 className="text-3xl font-semibold tracking-[-0.04em]">Connection control</h1><p className="mt-2 max-w-xl text-sm leading-6 text-muted-foreground">A focused control plane for import-ready Xray profiles on one HTTPS gateway.</p></div><div className="flex items-center gap-3"><Badge variant="outline" className="hidden border-emerald-500/30 bg-emerald-500/10 text-emerald-600 dark:text-emerald-300 sm:flex"><Activity className="mr-1.5 h-3.5 w-3.5" />Gateway online</Badge><ThemeControl /></div></header>

    <section className="mt-6 grid gap-4 md:grid-cols-3"><Metric icon={Network} label="Gateway transport" value="HTTPS / WebSocket" detail="One TLS edge, four private inbounds" /><Metric icon={ShieldCheck} label="Protocols" value="VLESS · VMess · Trojan" detail="SOCKS5 is available as Xray JSON" /><Metric icon={FileKey2} label="Subscription" value="Three URI profiles" detail="VLESS, VMess, and Trojan feed" /></section>
    ...
    <section className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_390px]"><div className="space-y-5"><div className="protocol-panel p-5 sm:p-7"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="protocol-overline">Gateway parameters</p><h2 className="mt-2 text-lg font-semibold">Public edge configuration</h2><p className="mt-1 text-sm text-muted-foreground">Changes update generated imports and restart the matching private listener.</p></div><span className="inline-flex w-fit items-center gap-1.5 rounded-full bg-emerald-500/10 px-3 py-1.5 text-xs font-medium text-emerald-600 dark:text-emerald-300"><Check className="h-3.5 w-3.5" />Database-backed</span></div><Separator className="my-6" /><form className="space-y-5" onSubmit={submit}><div className="grid gap-5 sm:grid-cols-[minmax(0,1fr)_130px]"><div className="space-y-2"><Label htmlFor="serverAddress">Server address</Label><Input id="serverAddress" value={form.serverAddress} onChange={event => setForm(value => ({ ...value, serverAddress: event.target.value }))} className="protocol-input" /></div><div className="space-y-2"><Label htmlFor="port">Port</Label><Input id="port" value={form.port} onChange={event => setForm(value => ({ ...value, port: event.target.value }))} className="protocol-input" inputMode="numeric" /></div></div><div className="space-y-2"><Label htmlFor="wsPath">VLESS WebSocket path</Label><Input id="wsPath" value={form.wsPath} onChange={event => setForm(value => ({ ...value, wsPath: event.target.value }))} className="protocol-input font-mono" /></div><div className="flex items-center justify-between rounded-xl border border-border bg-muted/35 px-4 py-3.5"><div><Label htmlFor="tlsEnabled" className="text-sm font-medium">TLS at the public edge</Label><p className="mt-1 text-xs text-muted-foreground">Advertise the managed HTTPS certificate in client imports.</p></div><Switch id="tlsEnabled" checked={form.tlsEnabled} onCheckedChange={tlsEnabled => setForm(value => ({ ...value, tlsEnabled }))} /></div><div className="flex justify-end"><Button type="submit" disabled={updateMutation.isPending}>{updateMutation.isPending ? <RefreshCw className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}Save gateway</Button></div></form></div>
      ...
      <aside className="space-y-5"><div className="protocol-panel p-5 sm:p-6"><p className="protocol-overline">Subscription feed</p><h2 className="mt-2 text-lg font-semibold">Single import surface</h2><p className="mt-1 text-sm leading-6 text-muted-foreground">Contains the VLESS, VMess, and Trojan connection profiles. SOCKS5 remains an Xray JSON import.</p><div className="mt-5"><CodePane label="Subscription URL" value={subscriptionUrl} qrDescription="Open subscription QR" onQr={() => setQrPayload({ name: "Subscription feed", description: "Scan to import VLESS, VMess, and Trojan profiles.", value: subscriptionUrl })} /></div><Button variant="outline" className="mt-3 w-full" onClick={() => setQrPayload({ name: "Subscription feed", description: "Scan to import VLESS, VMess, and Trojan profiles.", value: subscriptionUrl })}><QrCode className="mr-2 h-4 w-4" />Show subscription QR</Button><AlertDialog><AlertDialogTrigger asChild><Button variant="ghost" className="mt-2 w-full text-muted-foreground"><RefreshCw className="mr-2 h-4 w-4" />Rotate feed token</Button></AlertDialogTrigger><AlertDialogContent className="protocol-dialog"><AlertDialogHeader><AlertDialogTitle>Rotate subscription token?</AlertDialogTitle><AlertDialogDescription>The previous public subscription URL will immediately stop returning profile data.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction onClick={() => rotateToken.mutate()}>Rotate token</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog></div><div className="protocol-panel p-5 sm:p-6"><p className="protocol-overline">Protocol routes</p><div className="mt-4 space-y-3">{(Object.keys(protocolMeta) as ProtocolKey[]).map(key => { const path = key === "vless" ? profile.wsPath : key === "vmess" ? profile.vmess.wsPath : key === "trojan" ? profile.trojan.wsPath : profile.socks5.wsPath; return <button type="button" key={key} onClick={() => setActiveProtocol(key)} className="route-row"><span><span className={`protocol-dot ${protocolMeta[key].accent}`} />{protocolMeta[key].label}</span><code>{path}</code><ChevronRight className="h-4 w-4" /></button>; })}</div></div></aside></section>
    <section id="client-management" className="mt-8 scroll-mt-6"><ClientManagerContent /></section>
  </div></div>;
}
