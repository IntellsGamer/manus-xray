...
const [host, username, password, path = "/socks"] = process.argv.slice(2);
if (!host || !username || !password) {
  throw new Error("Usage: test-published-socks.mjs <host> <username> <password> [path]");
}

const xrayBinary = process.env.XRAY_BINARY_PATH || "/home/ubuntu/xray-validation/xray/xray";
const localPort = 18187;
...
async function main() {
  const workdir = await mkdtemp(join(tmpdir(), "published-socks-"));
  const configPath = join(workdir, "client.json");
  const config = {
    log: { loglevel: "warning" },
    inbounds: [{ listen: "127.0.0.1", port: localPort, protocol: "socks", settings: { auth: "noauth", udp: true, ip: "127.0.0.1" } }],
    outbounds: [{
      tag: "proxy",
      protocol: "socks",
      settings: { servers: [{ address: host, port: 443, users: [{ user: username, pass: password }] }] },
      streamSettings: {
        network: "ws",
        security: "tls",
  ...
  try {
    ...
    if (request.code !== 0 || !request.stdout.includes("Example Domain")) {
      throw new Error(`Published remote SOCKS5 transport failed: ${request.stderr || "unexpected response"}`);
    }
    console.log("Published remote SOCKS5 WebSocket transport passed.");
  } finally {
    client.kill("SIGTERM");
