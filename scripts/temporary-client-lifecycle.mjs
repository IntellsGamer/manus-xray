...
const testName = `Temporary 10 MB validation ${Date.now()}`;
...
async function main() {
  ...
  try {
    ...
    const created = await caller.createClient({ name: testName, trafficLimitBytes: quotaBytes, dayLimit: -1, speedLimitMbps: -1, connectionLimit: -1, creationRequestId });
    const retried = await caller.createClient({ name: testName, trafficLimitBytes: quotaBytes, dayLimit: -1, speedLimitMbps: -1, connectionLimit: -1, creationRequestId });
    ...
    if (created.trafficLimitBytes !== quotaBytes || created.dayLimit !== -1 || created.speedLimitMbps !== -1 || created.connectionLimit !== -1) throw new Error("The temporary client did not retain its unlimited day, speed, and connection policy");
    ...
    if (!persisted || persisted.trafficLimitBytes !== quotaBytes || persisted.dayLimit !== -1 || persisted.speedLimitMbps !== -1 || persisted.connectionLimit !== -1) throw new Error("The temporary client policy was not visible through the management API");
    ...
    if (!storedBeforeUsage) throw new Error("The temporary client could not be loaded for bridge-byte validation");

    await Promise.all([
      recordGatewayClientTunnelTraffic(temporaryId, quotaBytes / 2),
      recordGatewayClientTunnelTraffic(temporaryId, quotaBytes / 2),
    ]);
    const storedAfterUsage = await getGatewayClientById(temporaryId);
    if (!storedAfterUsage || storedAfterUsage.trafficUsedBytes !== quotaBytes) throw new Error("The real-time gateway meter did not persist the 10 MB bridge-byte total");
  ...
  console.log("Real temporary 10 MB client creation, concurrent route-keyed bridge-byte persistence, quota disablement, subscription rejection, permanent deletion, and existing-client preservation passed.");
  process.exit(0);
}
